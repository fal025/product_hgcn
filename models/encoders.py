"""Graph encoders."""
import manifolds
import layers.manifold_layers as manifold_layers
import numpy as np
import torch
...
class ManifoldNN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        dims, acts, _ = manifold_layers.get_dim_act_curv(args)
        hnn_layers = []
        for i in range(len(dims) - 1):
            in_dim, out_dim = dims[i], dims[i + 1]
            act = acts[i]
            hnn_layers.append(
                manifold_layers.ManifoldNNLayer(
...
class ManifoldGCN(Encoder):
    """
    A GCN on an arbitrary manifold.
    """
    def __init__(self, c, args):
        super().__init__(c)
        self.manifold = getattr(manifolds, args.manifold)()
        assert args.num_layers > 1
        dims, acts, self.curvatures = manifold_layers.get_dim_act_curv(args)
        ...
        for i in range(len(dims) - 1):
            ...
            act = acts[i]
            layers.append(
                manifold_layers.ManifoldGraphConvolution(
                    self.manifold,
                    in_dim, out_dim, 
                    c_in, c_out, 
                    args.dropout, 
                    act, args.bias, 
                    args.use_att, args.local_agg
                )
            )
        self.layers = nn.Sequential(*layers)
        self.encode_graph = True
